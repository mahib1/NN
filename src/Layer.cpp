...
DenseLayer::DenseLayer(int in_size, int out_size, int batchSize) {
    ...
    biases = Eigen::VectorXf::Random(out_size, batchSize); // random initialization
...
Eigen::MatrixXf DenseLayer::forward(const Eigen::MatrixXf& input) {
    ...
    input_cache = input; 
    Eigen::MatrixXf Z = weights * input + biases; // we get the Z matrix, 
    //Now we do the Thresholding! f(zi) = 1 if zi > threshold else 0

    Eigen::MatrixXf Act = Z.unaryExpr([](float z) {
...
//Tetsing Functions
void DenseLayer::DevDenseLayer(const Eigen::MatrixXf& w, const Eigen::MatrixXf& b) {
    weights = w;
    biases = b;
